...
i2c_master_dev_handle_t sht4x_device_create(i2c_master_bus_handle_t bus_handle, const uint16_t dev_addr, const uint32_t dev_speed)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "SHT4x device not found at address 0x%02X: %s", dev_addr, esp_err_to_name(ret));
        return NULL;
    }
    ESP_LOGI(TAG, "SHT4x device found at address 0x%02X", dev_addr);
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to add SHT4x device to I2C bus: %s", esp_err_to_name(ret));
        return NULL;
    }
