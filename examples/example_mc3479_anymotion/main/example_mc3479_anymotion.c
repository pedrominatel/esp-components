...
#define GPIO_INTN1                  4              /*!< GPIO pin for interrupt */
#define GPIO_INTN2                  5              /*!< GPIO pin for interrupt */
...
static void intr_task(void* arg)
{
    ...
    for (;;) {
        if (xQueueReceive(gpio_evt_queue, &io_num, portMAX_DELAY)) {
            printf("Interrupt on GPIO[%"PRIu32"] detected with value: %d\n", io_num, gpio_get_level(io_num));
        }
    }
}
...
static void mc3479_sensor_motion(void)
{
    ...
    err = mc3479_write_anymotion_threshold(sensor, 400);
    assert(ESP_OK == err);
    // Set the anymotion debounce
    err = mc3479_set_anymotion_debounce(sensor, 100);
    assert(ESP_OK == err);
...
static void mc3479_sensor_start(void)
{
    ...
    mc3479_config_t cfg;
    // Set the mode to sleep for the configuration
